...
class MypyBearTest(LocalBearTestHelper):
    ...
    def test_bad_language(self):
        ...
        while not self.queue.empty():
            ...
            msg = 'Language needs to be "Python", "Python 2" or "Python 3"'
            if message.message == msg:
                break
        else:
            assert False, 'Message not found'

    def test_check_untyped_defs(self):
        source = dedent("""
            def foo():
                return 1 + "abc"
        """).splitlines()
        self.check_validity(self.uut, source, valid=True)
        self.section.append(Setting('skip_untyped_functions', 'false'))
    ...
    def test_disallow_untyped_calls(self):
        source = dedent("""
            def foo():
                pass
