...
class CloneDetectionRoutines:
    ...
    def exclude_function(count_matrix):
        ...
        Applied heuristics:
        * Functions with only count vectors with a sum of all unweighted
        elements of lower then 10 are very likely only declarations or empty
        and to be ignored. (Constants are not taken into account.)
    ...
    def get_count_matrices(count_vector_creator,
        ...
        :param count_vector_creator: A object with a get_vectors_for_file
                                    method taking a filename as argument.
        :param filenames:            The files to create count vectors for.
        :param progress_callback:    A function with one float argument which
                                    is called after processing each file with
                                    the progress percentage (float) as an
                                    argument.
        :param extra_include_paths:  A list containing additional include
                                    paths.
        ...
        for i, filename in enumerate(filenames):
            ...
            for function in count_dict:
                if not CloneDetectionRoutines.exclude_function(count_dict[function]):
                    result[(filename,
                            function[0],
    ...
    def get_difference(matching_iterator,
                       ...
                       exp_postprocessing):
        """
        Retrieves the difference value for the matched function represented by
        the given matches.
        ...
        :param matching_iterator:   A list holding tuples of an absolute
                                    difference value and a value to normalize
                                    the difference into a range of [0, 1].
    ...
    def compare_functions(cm1,
        ...
        :param exp_postprocessing:  If set to true, the difference value of big
                                    function pairs will be reduced using an
                                    exponential approach.
        :return:                    The difference between these functions, 0
                                    is identical and 1 is not similar at all.
        """
        assert 0 not in (len(cm1), len(cm2))

        cm1, cm2 = CloneDetectionRoutines.pad_count_vectors(cm1, cm2)
        ...
        matching = CloneDetectionRoutines.munkres.compute(cost_matrix)

        return CloneDetectionRoutines.get_difference(
            [(diff_table[x][1][y][1], diff_table[x][1][y][2])
             for x, y in matching],
            average_calculation,
            poly_postprocessing,
            exp_postprocessing)
