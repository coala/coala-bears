...
def perform_upload(path):
    """
    Uploads the directory to PyPi, creating a ``sdist`` and a ``bdist_wheel``,
    after registering it.

    :param path: The folder in which the upload should be done.
    """
    subprocess.call([sys.executable, 'setup.py', 'register', '-r', 'pypitest'],
                    cwd=path)
    subprocess.call(
...
def main():
    ...
    for bear_file_name in glob.glob('bears/*/*Bear.py'):
