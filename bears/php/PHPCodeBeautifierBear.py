...
def get_configfile_path(name):
    return os.path.join(os.path.dirname(__file__), name)


def load_configfile(name):
    with open(get_configfile_path(name)) as f:
        return f.read()


@linter(executable='phpcbf',
        output_format='corrected',
        config_suffix='.xml',
        use_stdin=True,
        result_message='Formatting can be improved.')
class PHPCodeBeautifierBear:
    """
    Ensures that your PHP, JavaScript or CSS code remains clean and consistent.
    Provides PSR-1 standard by default.
    ...
    REQUIREMENTS = {
        AnyOneOfRequirements(
            [DistributionRequirement(apt_get='php-beautifier',
                                     zypper='php-pear-php_beautifier',
                                     ),
    ...
    CAN_DETECT = {'Formatting', 'Syntax', 'Documentation',
                  'Code Simplification'}
    CAN_FIX = {'Formatting', 'Syntax', 'Documentation',
               'Code Simplification'}

    @staticmethod
    def create_arguments(filename, file, config_file,
                         fix_error_and_warning: bool=False):
        args = ()
        if not fix_error_and_warning:
            args += ('-',)
        return args + ('--standard='+config_file, filename)
    ...
    def generate_config(filename, file,
        ...
        for k, v in rules_map.items():
            rules += '<rule ref="{}"/>\n'.format(k) if v else ''
        configs = load_configfile('psr-1.xml').format(
