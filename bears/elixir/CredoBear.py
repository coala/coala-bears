...
"""
Checks if ``mix`` and ``bash`` are installed
:return:
``True`` if ``mix``, ``bash`` and ``credo``
are installed otherwise ``False``
"""
...
@staticmethod
def create_arguments(self, filename, file, lintmode: str, config_file):
if lintmode:
